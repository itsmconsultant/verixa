...
def show_report_deposit_settlement_outstanding(conn):
    ...
    try:
        client_response = conn.client.schema("verixa").table("deposit_outstanding") \
            .select("client_id") \
            .eq("payment_at", tanggal_str) \
            .execute()
    ...
    if st.button("Tampilkan Data", use_container_width=True):
        ...
        with st.spinner(f"Mengambil data untuk {selected_client}..."):
            try:
                ...
                query = conn.client.schema("verixa").table("deposit_outstanding") \
                    .select("client_id, type, amount, service_fee_rate, service_fee_amount, final_fee, nett_amount, buy_rate_percent, buy_rate_cash, buy_nett_amount, nett_profit, payment_at, settle_at") \
                    .eq("payment_at", tanggal_str)
                ...
                response = query.order("client_id").execute()
